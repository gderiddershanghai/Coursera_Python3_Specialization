...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        coin_flip_result = random.randint(1, 10)
        return self.difficulty >= coin_flip_result
    ...
    def getPossibleLetters(self, guessed):
        ...
        if self.prizeMoney < VOWEL_COST:
            return [i for i in LETTERS if i not in guessed if i not in VOWELS]
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if len(possible_letters) == 0:
            ...
            frequent_unguessed = [i for i in self.SORTED_FREQUENCIES if i in possible_letters]
            if flip_result == False:
                return  random.choice(frequent_unguessed)
            elif flip_result == True:
                return frequent_unguessed[-1]
